...
#endif
  for(unsigned int i=0;i<m_nOfRow;i++)
  {
	SP=-1;
	DP=0;
...
// Functions:
	      ...
	      //we prefer the explicit acosh because of MS Visual C++ does not have acosh function  
	      case  cAcosh: if (Stack[SP] > -1 && Stack[SP] < 1)
						{ evalErrorType=4; return 0; }
			    Stack[SP] = log(Stack[SP]+sqrt(Stack[SP]*Stack[SP]-1)); break;
#endif
		  ...
		  //we prefer the explicit asinh because of MS Visual C++ does not have asinh function
		  case cAsinh: Stack[SP] = log(Stack[SP]+sqrt(Stack[SP] * Stack[SP]+1));
		  ...
		  //we prefer the explicit atanh because of MS Visual C++ does not have atanh function  
		  case cAtanh: if(Stack[SP] < 1 )
                       { evalErrorType=4; return 0; }
			    Stack[SP] = 1/2*log((1+Stack[SP])/(1-Stack[SP]));
#endif
...
//              Stack[++SP] = Vars[ByteCode[IP]-VarBegin];
              Stack[++SP] = (double) m_fArray[ByteCode[IP]-VarBegin][i];
        }
    }

    evalErrorType=0;
    m_result[i]=(float)Stack[SP];
  }
    return Stack[SP];
}
