...
#ifdef LIGHTCONE 
/*----------------------------------------------------------------------------
* c_backup: similar to c_part().
* Create block_size backup particles and initialize structure members to zero.. * This * function is essentially a cover function for calloc(block_size,
                                                                                                                                             * sizeof(oldparticle)). The position and momentum members of all 
* oldparticle structures are set equal to 0.0.
*----------------------------------------------------------------------------*/
bckptr c_backup(long block_size)
{
   bckptr   pptr;     /* pointer to particles                     */
   bckptr   i_pptr;   /* pointer to increment down block of particles */
   int       i;        /* coordinate index (X,Y,Z)                     */
   
   /* generate block_size particles */
   if((pptr = (bckptr) calloc(block_size, sizeof(oldparticle))) == NULL) {
      fprintf(stderr,"c_backup: error callocing particles\n");
      fflush(stderr);
      fclose(stderr);
      fprintf(io.logfile,"c_backup: error callocing particles\n");
      fflush(io.logfile);
      fclose(io.logfile);
      exit(1);
   }
   
   /* loop over nodes filling pos and mom with 0.0. */
...
void free_grid(gridls *cur_grid, int *no_grids)
{
  ...
  /* 'leavers' are reset whenever they were unstepped/advanced */
...
#endif
#ifdef GAS_PARTICLES
  cur_halo->prof.nvpart_gas   = (double*) calloc(nbins, sizeof(double));
  cur_halo->prof.nvpart_star  = (double*) calloc(nbins, sizeof(double));
#endif
}
...
#endif
#ifdef GAS_PARTICLES
  free(cur_halo->prof.nvpart_gas);
  free(cur_halo->prof.nvpart_star);
#endif
...
#endif
#ifdef GAS_PARTICLES
  cur_halo->prof.nvpart_gas   = NULL;
  cur_halo->prof.nvpart_star  = NULL; 
#endif
