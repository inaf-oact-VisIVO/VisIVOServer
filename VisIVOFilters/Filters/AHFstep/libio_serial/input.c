...
void read_mare_nostrum(FILE *icfile)
{
  ...
  long           n_cl_DM;
  int            i, k, i1, i6, i7, i14, SWAPBYTES;
...
#ifdef AHFmmfocus
               if(fabs(dummy[0]) < ZERO)
                 {
                  fprintf(stderr,"\n found a zero mass particle: #%ld with m=%g\n",j,dummy[0]);
                  fprintf(stderr,"     -> this will cause trouble with -DAHFmmfocus and hence we exit here...\n");
                  exit(0);
                 }
#endif
               k++;
         ...
         if(i == 0)
           {
            ...
            cur_gas->u = (flouble) P_gadget[ppart].u;
            cur_gas++;
           }
...
void read_ARTbinaries(FILE *fPMcrd, FILE *fPMcrs)
{
  ...
  if(fabs(ART_header.extras[59-1]) > ZERO)
    {
...
//  io.header.omegab      = 0.0;
//  io.header.gamma       = 0.0;
//  io.header.H_frac      = 0.0;  // if needed at all, these values are provided by the user and set in init_io_structure()
//  io.header.T_init      = 0.0;
//  io.header.B_init      = 0.0;
  io.header.pmass       = 0.0;    /* will be determined in ic_unit_conversion() */
  io.header.t_unit      = 0.0;    /* will be determined in ic_unit_conversion() */
...
void input(char *infile_name)
{
   ...
   fprintf(stderr,"no. of gas particles = %d\n",io.no_gas);
   fprintf(stderr,"gas particle offset  = %d\n",io.offset_gas);
   fprintf(stderr,"gas mass             = %g\n",(io.fst_part+io.offset_gas)->weight);
   fprintf(stderr,"DM min. mass         = %g\n",(io.fst_part+io.no_gas+1)->weight);
#endif
...
void read_data(FILE *icfile)
{
...
#ifdef LIGHTCONE 
  /* initiate the save set of old coordinates */
  io.fst_backup_part = c_backup(io.no_part);
  ...
  /* show that the lightcone file has not been opened yet */
  io.lightcone=NULL;
#endif /* LIGHTCONE */
