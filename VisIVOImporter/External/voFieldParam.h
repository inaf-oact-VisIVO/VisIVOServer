 ...
 *
 */
#ifndef voFIELDPARAM_H
#define voFIELDPARAM_H
...
/**
*
* This class was an abstract class, base class of Field and Param. 
*
* This class represented the Field/Param in the MetaData. 
...
//		int getLink (Link &link, int linkNum, int *status);
		...
		* in 'global.h'.
		*/
		int getDatatype (char * &datatype, int *status);
		...
		*/
		...
		int setDatatype(char *datatype, int *status);
...
//		vector<Link> m_linkList;
		...
		void cleanup(void);
		void makecopy(const voFieldParam &f);
		void init(void);
