...
//----------------------------------------------------------------------------
{
  ...
  this->InputScalarsSelectionXZ = NULL;
  this->InputScalarsSelectionY = NULL;
  this->InputVectorsSelection = NULL;
  this->InputNormalsSelection = NULL;
  this->UseSecondScalar = false;
  this->UseThirdScalar = false;
  this->UseCustomVectorScalar = false;
  this->CustomVectorScalar = NULL;
}
//----------------------------------------------------------------------------
ExtendedGlyph3D::~ExtendedGlyph3D()
 //----------------------------------------------------------------------------   
{
  ...
  this->SetInputScalarsSelectionXZ(NULL);
  this->SetInputScalarsSelectionY(NULL);
  this->SetInputVectorsSelection(NULL);
  this->SetInputNormalsSelection(NULL);
  this->SetCustomVectorScalar(NULL);
}

//----------------------------------------------------------------------------
void ExtendedGlyph3D::Execute()
 //----------------------------------------------------------------------------   
{
  try
  {
  ...
  vtkDataArray *inScalars_ScaleXZ = NULL;
  vtkDataArray *inScalars_ScaleY = NULL;
  ...
  float  vNew[3], s = 0.0, vMag = 0.0, value;
  double *v = NULL, *x;
  ...
  if(UseCustomVectorScalar)
    inScalars = pd->GetScalars(CustomVectorScalar);
  else if(!inScalars)
    inScalars = pd->GetScalars(NULL);
  
  if(UseSecondScalar)
	  inScalars_ScaleXZ = pd->GetScalars(this->InputScalarsSelectionXZ);
  if(UseThirdScalar)
	  inScalars_ScaleY = pd->GetScalars(this->InputScalarsSelectionY);
  ...
  if ( inScalars && !UseSecondScalar)
	{
	  inScalars->GetRange(RangeofScalarsXZ);
      ScalarNormalizationXZ = (RangeofScalarsXZ[1]-RangeofScalarsXZ[0]);

      if(ScalarNormalizationXZ != 0.0)
		{
	     ScalarNormalizationXZ = 1.0/ScalarNormalizationXZ;
		} else {
         ScalarNormalizationXZ = 1.0;
	     RangeofScalarsXZ[0] = 0.0;
		} 
	}
  else if(inScalars_ScaleXZ && UseSecondScalar)
  {
	  inScalars_ScaleXZ->GetRange(RangeofScalarsXZ);
      ScalarNormalizationXZ = (RangeofScalarsXZ[1]-RangeofScalarsXZ[0]);
  ...
  if(inScalars_ScaleY && UseThirdScalar)
  {
	  inScalars_ScaleY->GetRange(RangeofScalarsY);
      ScalarNormalizationY = (RangeofScalarsY[1]-RangeofScalarsY[0]);

      if(ScalarNormalizationY != 0.0)
		{
	     ScalarNormalizationY = 1.0/ScalarNormalizationY;
		} else {
         ScalarNormalizationY = 1.0;
	     RangeofScalarsY[0] = 0.0;
		} 
  ...
  for (inPtId=0; inPtId < numPts; inPtId++)
    ...
    if ( inScalars && !UseSecondScalar)
      ...
      if ( this->ScaleMode == VTK_SCALE_BY_SCALAR ||
        ...
        {
        scalex = scaley = scalez = (s-RangeofScalarsXZ[0])*ScalarNormalizationXZ;
		
        }
      }
	if ( inScalars_ScaleXZ && UseSecondScalar )
      {
      s = inScalars_ScaleXZ->GetComponent(inPtId, 0);
      if ( this->ScaleMode == VTK_SCALE_BY_SCALAR ||
           this->ScaleMode == VTK_DATA_SCALING_OFF )
        {
        scalex = scaley = scalez = ((s + 0.1) -RangeofScalarsXZ[0])*ScalarNormalizationXZ; // Fix better Zero size algorithm

        }
      }
	if ( inScalars_ScaleY && UseThirdScalar )
      {
      s = inScalars_ScaleY->GetComponent(inPtId, 0);
      if ( this->ScaleMode == VTK_SCALE_BY_SCALAR ||
           this->ScaleMode == VTK_DATA_SCALING_OFF )
        {
        scaley = (s-RangeofScalarsY[0])*ScalarNormalizationY;
...
//----------------------------------------------------------------------------
void ExtendedGlyph3D::ExecuteInformation()
  //----------------------------------------------------------------------------  
{
...
//----------------------------------------------------------------------------
void ExtendedGlyph3D::SetNumberOfSources(int num)
 //----------------------------------------------------------------------------   
{
...
//----------------------------------------------------------------------------
int ExtendedGlyph3D::GetNumberOfSources()
 //----------------------------------------------------------------------------   
{
...
//----------------------------------------------------------------------------
void ExtendedGlyph3D::SetSource(int id, vtkPolyData *pd)
 //----------------------------------------------------------------------------
{
  if (id < 0)
    {
...
//----------------------------------------------------------------------------
void ExtendedGlyph3D::ComputeInputUpdateExtents( vtkDataObject *output )
//----------------------------------------------------------------------------
{
  vtkPolyData *outPd;
